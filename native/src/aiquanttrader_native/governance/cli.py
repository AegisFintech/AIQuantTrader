    ...
    try:
        ...
        try:
            ...
            if args.command == "evaluate-canary":
                ...
                observation = CanaryObservation.model_validate_json(args.observation.read_bytes())
                report = evaluate_canary_evidence(
                    admission=evidence_record,
                    observation=observation,
                    policy=load_canary_policy(args.policy),
                )
                payload = report.canonical_bytes() + b"\n"
                if args.output is not None:
                    _atomic_write(args.output, payload)
                print(report.model_dump_json())
                return 0 if report.awaiting_production_approval else 1
        finally:
            ledger.close()
