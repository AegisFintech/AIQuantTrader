...
from aiquanttrader_native.sentinel.cli import main as sentinel_main
...
def test_execution_cli_wires_lifecycle_without_exposing_secret(
    ...
    monkeypatch.setattr("aiquanttrader_native.execution.cli.start_http_server", Mock())
    journal = Mock()
    monkeypatch.setattr("aiquanttrader_native.execution.cli.ExecutionJournal", lambda path: journal)
    monkeypatch.setattr("aiquanttrader_native.execution.cli.mark_stale_submissions", Mock())
    monkeypatch.setattr(
        "aiquanttrader_native.execution.cli.read_private_key", lambda path: "opaque-key"
    )
    built = SimpleNamespace()
    builder = Mock(return_value=built)
    runner = Mock()
    monkeypatch.setattr("aiquanttrader_native.execution.cli.build_trading_node", builder)
    monkeypatch.setattr("aiquanttrader_native.execution.cli.run_trading_node", runner)
...
def test_sentinel_cli_builds_independent_control_process(
    ...
    _enable_testnet(monkeypatch, tmp_path)
    monkeypatch.setattr("aiquanttrader_native.sentinel.cli.start_http_server", Mock())
    monkeypatch.setattr(
        "aiquanttrader_native.sentinel.cli.read_private_key", lambda path: "control-key"
    )
    client = object()
    monkeypatch.setattr(
        "aiquanttrader_native.sentinel.cli.HyperliquidControlClient",
...
def test_sentinel_metrics_healthcheck(
    ...
    monkeypatch.setattr(
        "aiquanttrader_native.sentinel.cli.urllib.request.urlopen",
