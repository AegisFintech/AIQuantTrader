...
from aiquanttrader_native.shadow.evidence import compare_shadow_runs
from aiquanttrader_native.shadow.gateway import ShadowGatewayService
from aiquanttrader_native.shadow.ingress import (
    ShadowIngressReader,
    ShadowIngressRecord,
    ShadowIngressWriter,
)
...
from aiquanttrader_native.shadow.service import ShadowClockError, ShadowEngineService
...
def test_gateway_run_uses_raw_recorder_and_publishes_terminal_states(
    ...
    monkeypatch.setattr("aiquanttrader_native.shadow.gateway.ManifestCatalog", FakeCatalog)
    monkeypatch.setattr("aiquanttrader_native.shadow.gateway.MarketDataRecorder", FakeRecorder)
    gateway = ShadowGatewayService(
        bundle=bundle,
        ingress=writer,
        registry=CollectorRegistry(),
