...
import aiquanttrader_native.governance.cli as governance_cli
...
def test_governance_cli_verifies_and_admits_through_explicit_controller_action(
    ...
    monkeypatch.setattr(
        "aiquanttrader_native.governance.cli.DeploymentAdmissionLedger",
        Mock(return_value=ledger),
    )
    monkeypatch.setattr(
        "aiquanttrader_native.governance.cli.configured_artifact_paths",
        Mock(return_value=object()),
    )
    verifier = Mock(return_value=admission)
    monkeypatch.setattr(
        "aiquanttrader_native.governance.cli.verify_deployment_admission",
        verifier,
    )
    identity = [
        "--config-dir",
