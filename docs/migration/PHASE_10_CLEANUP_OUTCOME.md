...
The top-level manifest binds the frozen retirement and credential-scan
policies, `mt5-final` source commit, archive, disabled report, schema-v3 cleanup
manifest, and exact preflight receipt. Its declared byte inventory must equal
...
Every target outcome records the exact approved target and pre-action state
hash, action start and completion, evidence capture time, collector, independent
reviewer, typed result, and raw-artifact references. Every raw artifact must be
captured after its action completes and before the reviewed target control.
Unreferenced or missing artifacts fail replay.
...
externally pinned Ed25519 trust root. It does not require that approval or
receipt to remain active during later audit.
...
2. action start inside the exclusive authority window;
3. exact approved target inventory;
4. approved pre-action state binding;
5. typed postcondition verification;
6. exact immutable outcome inventory;
7. complete policy-bound zero-finding scan;
8. required archive retention remaining; and
9. independent review for every target.
...
1. Regenerate any draft schema-v2 cleanup bundle under schema v3 and obtain a
   new signature; never reinterpret an old signature.
...
4. Attach the report, evidence hashes, and migration commit to the mechanical
   cleanup PR and retirement record.
...
Tests cover schema-v3 action compatibility, explicit migration destinations,
exact post-action inventory, historical preflight replay, start-time expiry,
canonical report loading, post-expiry independent replay, tampering, CLI
assembly/verification, JSON Schema export, and the no-executor architecture
boundary.
