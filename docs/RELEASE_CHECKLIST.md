...
- [ ] `ls -la .runtime/mt5/terminal/current/MQL5/Experts/AIQuantTrader/AIQuantTraderBridgeEA.ex5` — confirm mtime is fresh (just-compiled) and size > 100KB.
...
- [ ] `pm2 restart aiquanttrader-mt5 --update-env` — restart MT5 to load the freshly-compiled `.ex5`.
- [ ] `sleep 35` — wait for the EA to attach, the first timer tick, and a fresh heartbeat write to MT5 Common Files.

## 5. Post-deploy verification

- [ ] `pm2 list` — `aiquanttrader-mt5` is `online`, no new restart loop (restarts should not jump from 13 to 100s).
...
- [ ] `sudo scripts/install_cron.sh` to drop `config/aiquanttrader.cron` into `/etc/cron.d/aiquanttrader`
- [ ] `cat /etc/cron.d/aiquanttrader` to confirm the install
...
- [ ] `pm2 logs aiquanttrader-mt5 --lines 200` — inspect the live journal for the failure mode.
- [ ] `pm2 restart aiquanttrader-mt5 --update-env` — restart to roll back to whatever `.ex5` is on disk.
