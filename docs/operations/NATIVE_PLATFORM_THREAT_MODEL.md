...
| Forged or incomplete native-retirement observation | Legacy is stopped without continuous safe native operation | External pinned signer identity, independent signature/artifact/ledger reconstruction, exact renewal chain, typed risk reasons, reviewed incidents/drills, five-minute sentinel evidence bound | Deterministic reassembly failure; restart the observation when policy requires and retain both bundles |
| Incomplete, unrestorable, or credential-bearing legacy archive | Historical recovery fails or secrets survive cleanup | Exact eleven-category inventory, per-file hashes, isolated restore equality, externally pinned recursive zero-finding scan, annotated tag lineage, schema-v2 provenance | Archive assembly/replay failure; preserve sources, remediate findings, rebuild and independently review a new immutable bundle |
| Forged, stale, or selectively incomplete final MT5 state | Legacy is stopped with live/unmanaged positions, pending orders, or an active writer | Verified archive replay, raw report/broker/status/flag/writer hashes, independent reviews, account and position reconciliation, five-minute skew, one-hour freshness, schema-v2 provenance | Final-state assembly/replay failure or non-ready gates; preserve evidence, recapture all sources, and do not issue stop approval |
| Mixed, hand-authored, or stale retirement readiness | A valid native case is combined with another legacy archive, or authority/state expires between checks and report generation | Schema-v3 native retirement identity, dual-root assembly and replay, completion-time authority/freshness checks, canonical non-symlink input, evaluation-time source replay | Assembly/evaluation failure; preserve both roots, renew or recapture through reviewed upstream flows, and issue no stop approval |
| Hand-authored or discontinuous disabled observation | Legacy capability revival, post-stop orders, credential readers, native incidents, or secrets in raw evidence are hidden before cleanup | Exact six-control inventory, ordered stop proof, five-minute samples, complete broker/credential/native intervals, recursive zero-finding scan, action-time Ed25519 verification, current native/archive replay, evaluation-time source replay | Assembly/evaluation failure or an explicit failed gate; preserve evidence and issue no cleanup approval |
| Hand-authored, incomplete, or unsafe cleanup manifest | Legacy surfaces survive, changed paths are swept into deletion, shared packages are removed, or secret evidence is retained | Exact bundle inventory, independent nine-scope audit, typed state fingerprints, ownership and zero-shared-consumer proof, recursive zero-finding scan, disabled/archive lineage, deterministic assembly and replay | Assembly/replay failure; preserve disabled state, rebuild the evidence bundle, and issue no cleanup approval |
| Cleanup state changes after approval | An operator acts on a stale path, package, integration, or session inventory | Distinct post-approval full-bundle recapture, frozen five-minute freshness, exact inventory/state-hash comparison, active exact-scope signature, short-lived independently replayed evidence-only receipt | Preflight fails or expires; perform no action, recapture all state, and obtain a new approval if the approved inventory changed |
