...
# `AIQUANTTRADER_VALIDATOR_FUTURE_TOLERANCE_SECONDS` if you need a different bound.
import os

MAX_FUTURE_SECONDS = int(
    os.getenv("AIQUANTTRADER_VALIDATOR_FUTURE_TOLERANCE_SECONDS", "900")
)
KNOWN_ACK_STATUSES = {
    "OK",
    "ERROR",
    ...
    if ts_server <= MIN_VALID_TS or ts_server > int(time.time()) + MAX_FUTURE_SECONDS:
        issues.append(
            _warning(
                ...
                f"ts_server should be after 2020-01-01 and no more than {MAX_FUTURE_SECONDS} seconds in the future (override via AIQUANTTRADER_VALIDATOR_FUTURE_TOLERANCE_SECONDS)",
            )
        )
