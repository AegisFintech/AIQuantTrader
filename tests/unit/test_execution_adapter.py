...
from aiquanttrader.config.models import ExchangeNetwork, ExecutionConfig, RiskLimits
...
from aiquanttrader.execution.node import (
    BuiltTradingNode,
...
from aiquanttrader.risk import KillSwitchStore, RiskAuthority
...
def test_cancel_budget_and_terminal_lookup(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("aiquanttrader.execution.strategy.time.time_ns", lambda: NOW)
...
def test_node_factory_wires_only_hyperliquid_execution_owner(
    ...
    monkeypatch.setattr("aiquanttrader.execution.node.TradingNode", FakeNode)
    monkeypatch.setattr(
        "aiquanttrader.execution.node.RiskManagedExecutionStrategy",
        make_gateway,
    )
