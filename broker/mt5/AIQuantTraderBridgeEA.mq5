...
input string CommandFile = "aiquanttrader_commands.csv";
input string AckFile = "aiquanttrader_acks.csv";
input string StatusFile = "aiquanttrader_status.json";
input string PositionsFile = "aiquanttrader_positions.csv";
...
input string BlackoutFile = "aiquanttrader_blackout.csv";
...
bool WriteResearchBars() {
   ...
   if(copied <= 0) {
      Print("AIQuantTrader research export: CopyRates failed symbol=", symbol, " err=", GetLastError());
      return false;
   }

   int h = FileOpen(ResearchBarsFile, FILE_WRITE|FILE_TXT|FILE_ANSI|FILE_COMMON);
   if(h == INVALID_HANDLE) {
      Print("AIQuantTrader research export: FileOpen failed file=", ResearchBarsFile, " err=", GetLastError());
      return false;
   }
   ...
   for(int i = 0; i < copied; i++) {
      ...
      if(FileWriteString(h, line) <= 0) {
         ...
         FileClose(h);
         Print("AIQuantTrader research export: write failed file=", ResearchBarsFile, " err=", err);
         return false;
      }
   }
   FileClose(h);
   ...
   Print("AIQuantTrader research export: symbol=", symbol, " bars=", copied, " file=", ResearchBarsFile);
   return true;
}
...
void EnforceManagedRisk() {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--) {
      ...
      if((sl <= 0.0 || tp <= 0.0) && StringFind(comment, "AIQuantTrader_") != 0) {
...
void PollCommands() {
   ...
   if(h == INVALID_HANDLE) {
      if(commandFileErrLogged == 0) {
         Print("AIQuantTraderBridgeEA: command file not found yet: ", CommandFile, " err=", GetLastError());
   ...
   while(!FileIsEnding(h)) {
      ...
      if(n < 9) {
         Print("AIQuantTraderBridgeEA: malformed command line: ", line);
         continue;
      }
...
void ManageAutoSymbol(string symbol, int idx) {
   ...
   trade.SetDeviationInPoints(DefaultDeviationPoints);
   bool ok = side > 0
      ? trade.Buy(volume, symbol, 0.0, sl, tp, "AIQuantTrader_" + symbol + "_" + reason)
      : trade.Sell(volume, symbol, 0.0, sl, tp, "AIQuantTrader_" + symbol + "_" + reason);
   if(ok) {
